#This script contains Spatial 2 model with informative theta and tau~uniform(0,1000) for tau priors sensitivity
...
model{
  ...
  theta ~ dnorm(5.984782,0.01571683) #informative prior from glm estimate
  theta2 ~ dt(0.2050,14.34982,7)#informative prior from glm estimate
  tau  <- pow(sigma.theta2,-2)
  sigma.theta2~dunif(0,1000)#change to uniform to assess sensitivity
  sigsq <- pow(tau,-1)
  phi ~ dunif(0.05, 15)
  kappa ~ dunif(0.05,1.95) 
  
}
model.file <- file.path("~/Bayesian_final_report/Model_script/county_level/Spatial/Spatial_2_uniform.txt") 
...
inits <- function(){
  list(theta=rnorm(1,2,2), theta2=rnorm(1,0,2),phi=1,sigma.theta=runif(55,0,10),
       sigma.theta2=runif(1,0,10),kappa=1)
}
